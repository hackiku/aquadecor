...
// 1. DELETE the 'interface AdditionalItem'. 
// We will let TRPC infer the type automatically.
			...
			<div className={cn(
				...
				{items.map((item) => (
					<AdditionalItemCard
						...
						id={item.id}
						name={item.name}
						// FIX 1: Allow null/undefined description
						description={item.description || undefined}
						priceCents={item.priceCents}
						// FIX 2: Fallback for null image
				...
				<div className="p-4 bg-accent/5 rounded-xl border">
					...
					<ul className="text-xs text-muted-foreground space-y-1">
						{Object.entries(addedItems).map(([itemId, qty]) => {
							// FIX 3: Remove the explicit ': AdditionalItem' type annotation
							// TypeScript knows what 'items' is because of tRPC.
