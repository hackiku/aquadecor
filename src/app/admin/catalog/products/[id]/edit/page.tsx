// src/app/admin/catalog/products/[id]/edit/page.tsx
...
export default function EditProductPage({ params }: PageProps) {
	...
	const { data: product, isLoading } = api.admin.product.getById.useQuery({ id });
	...
	useState(() => {
		if (product) {
			setFormData({
				...
				stockStatus: (product.stockStatus as any) || "made_to_order",
	...
	const updateProduct = api.admin.product.update.useMutation({
		onSuccess: () => {
			...
			router.push(`/admin/catalog/products/${id}`);
		},
		onError: (error) => {
	...
	if (!product) {
		return (
			<div className="space-y-8">
				<h1 className="text-4xl font-display font-extralight">Product not found</h1>
			</div>
		);
	}
		...
		<div className="space-y-8">
			...
			<div className="space-y-4">
				<Button variant="ghost" asChild className="font-display font-light -ml-4">
					<Link href={`/admin/catalog/products/${id}`}>
