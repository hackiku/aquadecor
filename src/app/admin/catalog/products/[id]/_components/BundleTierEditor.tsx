// src/app/admin/catalog/products/[id]/_components/BundleTierEditor.tsx
"use client"
...
export function BundleTierEditor({ pricingId, bundles, onUpdate }: BundleTierEditorProps) {
	...
	const formatPricePerPiece = (totalCents: number, quantity: number) => `€${(totalCents / 100 / quantity).toFixed(2)}`
			...
			<CardContent className="space-y-4">
							...
							<div className="flex-1 space-y-2">
								<div className="flex items-center gap-3">
									<div className="flex items-center gap-2">
										<span className="text-2xl font-display font-light">{bundle.quantity}</span>
										<span className="text-sm text-muted-foreground font-display font-light">pieces</span>
									</div>
									{bundle.isDefault && (
								...
								<div className="flex items-baseline gap-3">
									<span className="text-xl font-display font-semibold">{formatPrice(bundle.totalPriceEurCents)}</span>
				...
				{isAddingNew && (
					<div className="p-4 rounded-lg border-2 border-dashed border-primary/50 space-y-4">
						<div className="grid gap-4 sm:grid-cols-2">
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) => setNewBundle({ ...newBundle, quantity: Number.parseInt(e.target.value) || 0 })}
									placeholder="e.g., 7"
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setNewBundle({
											...newBundle,
											totalPriceEurCents: Math.round(Number.parseFloat(e.target.value) * 100) || 0,
										})
									}
						...
						<div className="grid gap-4 sm:grid-cols-2">
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) => setNewBundle({ ...newBundle, savingsPercent: Number.parseInt(e.target.value) || 0 })}
									placeholder="e.g., 15"
									className="font-display font-light"
								/>
							</div>
						...
						<Button onClick={handleCreateBundle} disabled={createBundle.isPending} className="w-full rounded-full">
