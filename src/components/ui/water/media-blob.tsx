...
interface MediaBlobProps {
	...
	duration?: number;
	/** Size mode: 'auto' for 30vw square, 'fill' for container-fill, 'viewport' for full viewport */
	sizeMode?: "auto" | "fill" | "viewport";
}
...
export function MediaBlob({
	...
	// Determine size class based on mode
	const sizeClass = {
		auto: "w-[30vw] h-[30vw] min-w-[300px] min-h-[300px]",
		fill: "w-full h-full",
	...
	return (
		<div
			className={cn("blob relative", sizeClass, className)}
			style={
				{
					"--time": `${duration}s`,
					"--amount": amount,
		...
		>
			{type === "image" ? (
				<Image
					...
					priority={priority}
					sizes={sizeMode === "viewport" ? "100vw" : sizeMode === "fill" ? "100vw" : "(max-width: 768px) 100vw, 30vw"}
				/>
			) : (
				<video
					autoPlay={autoPlay}
